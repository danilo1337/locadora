...
public class PdfFilmes extends PdfTemplate {
	...
	public PdfPTable gerarTabela() throws SQLException {
		...
		for (Object o : listaDeObjetos) {
			...
			celula.add(
					new PdfPCell(new Paragraph(p.getFaixaEtaria(), FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
			celula.add(new PdfPCell(
					new Paragraph(p.getAnoLancamento(), FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
		}
