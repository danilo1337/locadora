...
public class PdfLocacao extends PdfTemplate{
	String tituloCelulas[] = null;
	PdfPTable tabela = null;
	List<PdfPCell>celula = null;
	List<?> listaDeObjetos = null;
	public PdfLocacao(String nome_pdf,String tituloCelulas[],List<?> listaDeObjetos) {
		super(nome_pdf);
	...
	public PdfPTable gerarTabela() throws SQLException {
		...
		for (Object o: listaDeObjetos ) {
			...
			celula.add(new PdfPCell(new Paragraph(p.getPessoal().getNomeCompleto(),FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
			celula.add(new PdfPCell(new Paragraph(p.getDataLocacao()+"",FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
			celula.add(new PdfPCell(new Paragraph(p.getDataPagamento()+"",FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
			celula.add(new PdfPCell(new Paragraph(p.getValorTotal()+"",FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
			celula.add(new PdfPCell(new Paragraph(p.getMulta()+"",FontFactory.getFont(FontFactory.TIMES_ROMAN, 10))));
		}
